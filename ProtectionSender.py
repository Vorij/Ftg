...
Distributing without author's tag is strictly prohibited by license"""
    strings = {
        ...
        'antichannel_on': '📯 <b>AntiChannel on</b>\n🇺🇸 <b>Action: Disallow writing on behalf of channels</b> 👊🏼\n🇷🇺 <b>Действие: Запретить писать от имени каналов</b> 👊🏼',
        'antichannel_off': '📯 <b>AntiChannel off</b>',
        'antichannels': '📯 <code>{}</code>\n🇺🇸 <b>Anonymous wrote on behalf of the channel</b>\n👊🏼 <b>Action: Forbade him to write on behalf of the channels</b>\n🇷🇺 <b>Аноним написал от имени канала</b>\n👊🏼 <b>Действие: Запретил ему писать от имени каналов</b>'
    }
    ...
    async def client_ready(self, client, db):
        ...
        async def edited_handler(event):
            ...
            user = message.sender.id if getattr(message, 'sender, None) is not None else if getattr(message.sender, 'id', None) is not None else 0
    ...
    async def arabcmd(self, message):
        ...
        if 'arabshield' not in self.chats[chat]:
            self.chats[chat]['arabshield'] = 'ban'
            await new_answer(message, self.strings('as_on', message).format('ban'))
        else:
    ...
    async def pchatscmd(self, message):
        ...
        res = f"<b><u>🦊 Chat Protection</u></b> <i>{version}</i>\n\n<i>🐼 - AntiLogspam\n🐺 - AntiHelp\n🐻 - AntiArab\n🐵 - AntiTagAll\n💋 - AntiSex\n🚪 - AntiRaid\n📯 - AntiChannel\n\n👋 - Welcome\n👮‍♂️ - Warns</i>\n\n🦊 <b><u>Chats:</u></b>\n"
        changes = False
    ...
    async def punish(self, cid, user, violation, action, user_name):
        ...
        if action == "delmsg":
            msgdel = await self.client.send_message(cid, self.strings(violation).format(user, user_name, 'deleted message'))

            await msgdel.delete()
        elif action == "kick":
            msgdel = await self.client.send_message(cid, self.strings(violation).format(user, user_name, 'kicked him'))
            await self.client.kick_participant(cid, user)
            await msgdel.delete()
        elif action == "ban":
            msgdel = await self.client.send_message(cid, self.strings(violation).format(user, user_name, 'banned him for 2 day'))
            await self.client(telethon.tl.functions.channels.EditBannedRequest(cid, user, telethon.tl.types.ChatBannedRights(until_date=time.time() + 3600 * 48, view_messages=True, send_messages=True, send_media=True, send_stickers=True, send_gifs=True, send_games=True, send_inline=True, embed_links=True)))
            await msgdel.delete()
        elif action == "mute":
            msgdel = await self.client.send_message(cid, self.strings(violation).format(user, user_name, 'muted him for 2 day'))
            await self.client(telethon.tl.functions.channels.EditBannedRequest(cid, user, telethon.tl.types.ChatBannedRights(until_date=time.time() + 3600 * 48, send_messages=True)))
            await msgdel.delete()
        elif action == "warn":
            if not self.warn:
    ...
    async def warnscmd(self, message):
        ...
        if not await check_admin(message.sender.id):
            await send_user_warns(message.sender.id)
        else:
            reply = await message.get_reply_message()
            args = utils.get_args_raw(message)
    ...
    async def antichannelcmd(self, message):
        ...
        if 'antichannel' not in self.chats[cid]:
            ...
            await new_answer(message, self.strings('antichannel_on', message))
        else:
            del self.chats[cid]['antichannel']
            await new_answer(message, self.strings('antichannel_off', message))
    ...
    async def watcher(self, message):
        ...
        try:
            ...
            user = message.sender.id or 0

            if 'defense' in self.chats[cid] and user in self.chats[cid]['defense']:
                return

            try:
                if (await self.client.get_permissions(int(cid), message.sender.id)).is_admin: return
            except: pass
            ...
            if 'antichannel' in self.chats[cid]:
                if message and message.sender and message.sender.has_link is False:
                    await message.delete()
                    await self.client(telethon.tl.functions.channels.EditBannedRequest(int(cid), int(user), telethon.tl.types.ChatBannedRights(until_date=time.time() + 3600 * 48, view_messages=True, send_messages=True, send_media=True, send_stickers=True, send_gifs=True, send_games=True, send_inline=True, embed_links=True)))
                    await self.client.send_message(int(cid), self.strings('antichannels').format(user))
                    return
            ...
            if 'arabshield' in self.chats[cid]:
                to_check = getattr(message, 'message', '') + \
                    getattr(message, 'caption', '') + (message.forward.chat.title if getattr(message, 'forward', None) is not None and getattr(message.forward, 'chat', None) is not None and getattr(message.forward.chat, 'title', None) is not None else '')
                if message.sender.bot and message.forwards or len(re.findall('[\u4e00-\u9fff]+', to_check)) != 0 or len(re.findall('[\u0621-\u064A]+', to_check)) != 0:
