...
class ModulesLinkMod(loader.Module): 
    ...
    async def mlcmd(self, message): 
        ...
        if not args: 
            return await message.edit("<code>.ml ModulesLink</code>") 
        ...
        try: 
            ...
            if "http" not in link: 
                ...
                text = f"<a href=\"{link}\">Ссылка</a> на {f}: <code>.dlmod {link}</code>" 
            ...
            return await message.edit("Попробуй ввести имя модуля без кавычек")