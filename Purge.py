# by @Friendly_telegram_userbot - @IO3EP
...
class PurgeMod(loader.Module):
    """Удаляет сообщения"""
    strings = {"name": "Purge",
               "from_where": "<b>Ответь на то сообщение до которого нужно удалить сообщения</b>",
               "not_supergroup_bot": "<b>Чистить можно только в супергруппах</b>",
               "delete_what": "<b>Ответь на сообщение которое нужно удалить</b>"}

    @loader.group_admin_delete_messages
    @loader.ratelimit
    async def purgecmd(self, message):
        """Удаляет до ответного сообщения"""
