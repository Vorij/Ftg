...
class FileUploaderMod(loader.Module):
	...
	async def x0cmd(self, message):
		""".x0 <Ответ на медью> - Загружает в x0"""
		await message.edit("<b>В x0 загружаю...</b>")
		reply = await message.get_reply_message()
		if not reply:
			await message.edit("<b>Ответьте на медью</b>")
			return
		media = reply.media
		if not media:
		...
		try:
			...
			await message.edit("<b>Ошибка:</b> <code>.logs 40</code>")
			return
		url = x0at.text
		output = f'<a href="{url}">Ссылка:</a> <code>.dlmod {url}</code>'
		await message.edit(output)

	async def phcmd(self, message):
		""".ph <Ответ на фото или видео либо гиф> - Загружает в telegraph"""
		if message.is_reply:
			reply_message = await message.get_reply_message()
			data = await check_media(reply_message)
			if isinstance(data, bool):
				await message.edit("<b>Ответьте на фото или видео либо гифку</b>")
				return
		else:
			await message.edit("<b>Ответьте на фото или видео либо гифку</b>")
			return
	...
	async def imgurcmd(self, message):
		""".imgur <Ответ на фото> - Загружает в imgur"""
		chat = '@ImgUploadBot'
		reply = await message.get_reply_message()
		async with message.client.conversation(chat) as conv:

			if not reply:
				await message.edit("<b>Ответьте на фото</b>")
				return
			else:
				pic = await check_mediaa(message, reply)
				if not pic:
					await utils.answer(message, '<b>Ответьте на фото</b>')
					return
			await message.edit("<b>В imgur загружаю...</b>")
			try:
				what = lol(pic)
				...
				return
			await message.edit("<b>imgur ссылка:</b>" + response.text)
